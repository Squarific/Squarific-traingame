SQUARIFIC = SQUARIFIC || {};
SQUARIFIC.train = SQUARIFIC.train || {};
...
SQUARIFIC.train.World = function World (server, settings, trains, tracks, client) {
	...
	this.addSocket = function (socket) {
		...
		socket.emit("worldData", {settings: settings, trains: this.trainController.trains, tracks: this.tracks.tracks});
	};	
	...
	this.getChunkData = function (chunk) {
		...
		for (var x = 0; x < settings.chunkSize; x++) {
			for (var y = 0; y < settings.chunkSize; y++) {
				if (Math.random() < 0.0002 && this.map.getBlockData(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y) > 0.53 && !this.citys.getCityNear(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y, 150) && !this.industrys.getIndustryNear(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y, 90)) {
					this.citys.newCity(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y);
				}
				if (Math.random() < 0.0001 && this.map.getBlockData(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y) > 0.53 && !this.citys.getCityNear(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y, 90) && !this.industrys.getIndustryNear(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y, 130)) {
					this.industrys.newIndustry(chunk[0] * settings.chunkSize + x, chunk[1] * settings.chunkSize + y);
				}
			}
		}
