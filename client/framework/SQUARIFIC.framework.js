...
SQUARIFIC.framework.Chunks = function (settings, getXYcolor) {
	...
	this.fillCanvas = function (ctx, cx, cy) {
		var rgba = ctx.createImageData(settings.chunkSize, settings.chunkSize);
		for (var x = 0; x < settings.chunkSize; x++) {
			for (var y = 0; y < settings.chunkSize; y++) {
				var color = getXYcolor(cx * settings.chunkSize + x, cy * settings.chunkSize + y);
				var o = (y * settings.chunkSize + x) * 4;
				rgba.data[o] = color[0];
				rgba.data[o + 1] = color[1];
				rgba.data[o + 2] = color[2];
				rgba.data[o + 3] = 255;
			}
		}
...
SQUARIFIC.framework.Captions = function Captions (captions) {
	this.rgbFromNumber = function cssRgbFromNumber (number) {
		var r = 0, g = 0, b = 0;
...
SQUARIFIC.framework.Renderer = function (ctx, settings) {
    this.layers = [];
    this.layer = {
        simulatedX: 0,
        simulatedY: 0
    };
    this.newLayer = function (layer, position) {
        this.layers.push({
            layer: layer,
            position: position
        });
    ...
    this.render = function render () {
        for (var key = 0; key < this.layers.length; key++) {
            this.layers[key].paint(ctx, this.layer);
        }  
    };
...
SQUARIFIC.framework.Gui = function Gui (container) {
	...
	this.addWindow = function addWindow (title, options) {
		...
		topBar.addEventListener("mousedown", this.mousedown.bind(this, title), false);
		document.addEventListener("mouseup", this.mouseup.bind(this, title), false);
		...
		document.addEventListener("touchend", this.mouseup.bind(this, title), false);
		topBar.appendChild(titlediv);
		close.innerText = "X";
		close.className = "window-close";
		close.addEventListener("click", options.closeButton, false);
...
if (typeof model !== "undefined" && model.exports) {
	model.exports = SQUARIFIC.framework;
}